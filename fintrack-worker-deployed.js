--786b5e8580a4cb22eb7784b0f5bfbfe28d50f4d50e782ca9e11089f07f2c
...
async function createTransaction(request, env, user) {
  ...
  const body = await request.json();
  const { name, amount, type, scope, date, note, walletId, categoryId, subCategoryId } = body;
  ...
  await env.DB.batch([
    env.DB.prepare(
      "INSERT INTO transactions (id, workspace_id, created_by_user_id, wallet_id, category_id, sub_category_id, name, amount, type, scope, date, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(id, user.workspace_id, user.id, resolvedWalletId, categoryId || null, subCategoryId || null, name, amt, type, scope, date, note || null),
...
async function uploadSlip(transactionId, request, env, user) {
  const tx = await env.DB.prepare("SELECT id, workspace_id FROM transactions WHERE id = ? AND workspace_id = ?").bind(transactionId, user.workspace_id).first();
  ...
  await env.DB.prepare(
    "INSERT INTO slips (id, workspace_id, transaction_id, file_key, file_name, file_size, mime_type, slip_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  ).bind(slipId, user.workspace_id, transactionId, fileKey, fileName, body.byteLength, contentType, slipType).run();
  return json({ slip: { id: slipId, fileName, slipType, mimeType: contentType, fileSize: body.byteLength } }, 201);
}
